/*
DOMContentLoaded : 화면에 DOM 요소들이 모두 완성되면..실행하라는 코드
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const createLiTag = (todoContent) => {
    ...
    //위에서 만들어진 span tag에 각각 class 를 부착하라
  ...
  /*
    ...
    요청을 하면 화면의 최초로 발견되는 요소만 select 가 된다
    */
  ulContent.addEventListener("click", (event) => {
    ...
    이 화면에서 실제 event.target 은 SPAN tag 가 된다
    그러면 SPAN 이 아닌 LI tag에 대하여 어떤 처리를 하려고 한다
    */
    ...
    if (target.tagName === "SPAN") {
       ...
       * 배열로 저장되어 있다
       * 즉, liTag(LI) 에 class 속성이 있고, 값이 complete 이면 제거 만약 없으면 추가
       *
       * 클래스변수 ?.aaa : 클래스 변수가 null 이 아닌경우에만 aaa 코드실행
